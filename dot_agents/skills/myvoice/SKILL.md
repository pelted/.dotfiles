...
name: myvoice
description: |
  Write or rewrite text in my voice. Use whenever drafting or editing
  prose I will use or share: Slack messages, PR descriptions, commit bodies,
  review and issue comments, emails, docs, and longer explanations. Loads my
...
3. **Write like the samples** - Match what's actually there, not what a polished version of me would sound like (see What to match).
...
My real writing lives at `~/.context/voice/` (the private context-global repo, so it travels to every machine):
...
| `rewrites.md` | Before/after pairs, agent draft vs. my rewrite - the highest-signal data here |
...
- Word choice level - if I write "stuff" and "things," don't upgrade to "elements" and "components"
- How I open messages and paragraphs (jump right in? Set context first?)
...
- How I handle transitions, soften asks, and push back
...
- I explicitly ask for a neutral, structured, or "AI" voice
...
- When I rewrite something you drafted, that pair is gold: offer to append it to `rewrites.md` (date, genre, one-line context, your draft, my rewrite verbatim, and an optional **Note:** stating the generalizable lesson when the pair encodes a rule that isn't obvious from the diff).
- Never harvest new samples from GitHub PRs or comments authored after ~Feb 2025 - that text is largely agent-written, not my voice. Commit and merge messages are excluded from any era: my commit messages have been agent-written for years, and my merge messages are quips unrelated to the change. `rewrites.md` is where current PR/commit voice signal accrues.
- Slack has no date cutoff (I type it myself), but DMs stay out of the corpus - channel messages only.
...
**After:**
> I want to run this in staging for a few days and watch the error rate before rolling to prod.

**Before:**
> Once it has soaked, we can ramp the flag.
...
**Never include in PR descriptions, commit messages, or any public-facing artifact:**
- Customer or organization names
- Organization IDs, account IDs, tenant IDs, munemos
- Person names, emails, user IDs
- Counts that could fingerprint a specific customer ("the org with 4,217 devices")
- Internal customer-specific incident references that name the customer

**Problem:** PR descriptions and commit messages are public artifacts (or treated as such within the company). Customer-identifying info leaks into search results, screenshots, public mirrors, and external eyes. Describe the *shape* of the problem, not the *identity* of who hit it.

**Before:**
> Fixes a bug reported by Acme Corp (org_id: 12345) where their 4,217-device fleet was timing out on the agents page.

**After:**
> Fixes a timeout on the agents page for orgs with several thousand devices.

### P3. Drop "Test plan" sections

**Problem:** "Test plan" is a template artifact from AI scaffolds and PR bots. It encourages vague checklists ("test the happy path", "test edge cases") that age into noise. If you tested something, write what you tested in plain prose. If you didn't, don't pretend you did.
...
**After:**
> Ran the migration locally against a snapshot of the staging DB. Confirmed the new index gets picked up by the slow query I was targeting. I did not test the rollback path.

If a section heading is genuinely useful, use "What I tested" or "Verification" and write actual sentences underneath.

### P4. No em dashes, ever, in PRs and comments

**Problem:** Rule 14 already covers em dashes in general. In PR descriptions and comments specifically, treat this as a hard rule with zero exceptions. Em dashes are the single loudest AI tell in short-form writing. Replace with comma, period, colon, parenthesis, or a connecting word per the replacement guide in section 14.

**Before:**
> The fix is small — just one line in the validator.

**After:**
> The fix is small: one line in the validator.

**Before:**
> This regressed in #14376 — we missed it because the test only covered the happy path.

**After:**
> This regressed in #14376. The test only covered the happy path, so we missed it.

### P5. No marketing closers

**Phrases to watch:** This unlocks, this paves the way, this sets us up to, this is a step toward, this lays the groundwork

**Problem:** PR descriptions don't need an inspirational closer. State what changed and stop.

**Before:**
> This refactor unlocks future work on the permissions system and sets us up to move faster on the next milestone.

**After:**
> This refactor pulls the permission check into its own service object so we can swap implementations later.
